...
**This is a Windows-only fork** of [ResearchMesh](https://github.com/nodormu/ResearchMesh), taken at commit `9e6959b`. Windows is assumed outright: there is not a single `sys.platform` check left in the app, and the only one anywhere is in `smoke_test.py`, which has to know where it is running in order to skip. Do not add cross-platform branches — if something reads as a POSIX leftover, it is a bug, not compatibility.

Two things follow from the fork that are easy to trip over:

- **There is no `bash` tool.** Anthropic's learned `bash_20250124` schema was removed and `core/powershell.py` replaces it. That is not a rename: `bash` is a *learned* schema, so Claude is trained to emit POSIX shell for that tool name, and no interpreter swap underneath it can be honest about that. Keeping real bash would have meant depending on Git for Windows and living with its MinGW filesystem view (`/c/Users/...`) disagreeing with the native paths every other tool here emits and accepts. `powershell` is a custom schema for exactly this reason.
- **Nothing here has been run by hand on Windows.** The whole port was written on a Linux box. `ruff`, `mypy` and `smoke_test.py` pass, and CI's `check` job runs on `windows-latest` — which makes GitHub Actions the only place this code has executed on its target platform at all. Treat a green run as "it imports, wires up and handshakes", which is well short of "the tools work". `document_convert`, `interactive_run` and `computer` are the three most likely to still need a real session.

`README.md` has **not** been updated and still describes the Linux original; it carries a banner saying so. Where the two disagree, this file and the `core/` module docstrings are correct.
...
There is no `playwright install-deps` step: that installs Linux shared libraries and does not apply here.
...
The `computer` tool needs no system packages. The Linux original required `python3-tk` and `scrot` and an X11 display, with a Wayland refusal on top; on Windows `pyautogui` and `pillow` are the whole dependency and screen capture works out of the box. What replaces the Wayland problem is **UIPI**: a process at medium integrity cannot send input to a window owned by an elevated one, so if anything running as Administrator has focus, clicks and keystrokes are discarded and the screenshot afterwards looks exactly like a click that missed. It is not refused up front the way Wayland was, because it depends on which window has focus at that instant rather than on the session — see the `core/computer.py` bullet under Architecture.
...
**Do not send anyone to `README.md` for setup yet.** It still documents the Linux original — `apt` packages, X11, a `bash` tool that no longer exists — and has not been rewritten for this fork. The Commands section above is the current instruction; the `core/` module docstrings are the current detail. (Upstream, `SETUP.md` was merged into the README because the two duplicated ~60% of their content and drifted apart.)
...
**CI's `check` job runs on `windows-latest`.** The `secrets` job stays on `ubuntu-latest`: it
greps the repository and never imports the code, so the platform is irrelevant to what it
proves, and it is a bash script that would need rewriting for PowerShell to prove the same
thing. Note the default shell on a Windows runner is PowerShell, where `\` is not a line
continuation — a wrapped `run:` block written the POSIX way passes a bare backslash to the
command.
...
`core/computer.py`'s `ctypes.windll` calls type-check from a Linux box, and it cuts the more
useful way too: code that is wrong *on Windows* is reported even where it would otherwise pass.
...
**Neither of them passes `shell=True`, and that matters more on Windows than it did on Linux.**
...
- `CLAUDE_COMPUTER_FORCE` — **gone.** It existed only to bypass the Wayland refusal, and there is no refusal to bypass. If you find it referenced anywhere, that reference is stale.
...
- **A learned tool is exempt from the "must beat the shell" test below** — the schema already exists in the model, so the only question is whether you want the capability, not whether it earns a slot on novelty. The converse is the lesson of this fork: **a learned schema is only an asset while its dialect matches the machine.** `bash_20250124` cost nothing to keep on Linux and became actively misleading here, because the training that made it free is training to emit POSIX shell.
...
between them. Removing `notify` needed every one of these, and so did swapping `bash` for
`powershell` in this fork; missing any leaves a doc that lies or a prompt that names a tool
Claude doesn't have. Touch them all:
