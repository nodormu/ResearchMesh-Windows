...
- **Untested.** Every line of the port was written on a Linux box and has never executed on Windows. `ruff`, `mypy` (now with `platform = "win32"`, so it checks as Windows) and `smoke_test.py` all pass, but those check wiring, not behaviour.
- **`.github/workflows/ci.yml` still runs on `ubuntu-latest`**, which will now fail to install `pywinpty`.
