...
The refusals are in the `__main__` path only, so importing either module still works anywhere; `smoke_test.py` depends on that. They exist because without them the app starts fine and then fails one tool at a time — no PowerShell interpreter, no pywinpty, `ctypes.windll` missing, no LibreOffice — and every one of those reads as a broken install rather than as the wrong operating system.
...
- **This has since been run by hand on a real Windows box.** `ruff`, `mypy` and `smoke_test.py` pass, and CI runs on `windows-latest` — a green run there only ever meant "it imports, wires up and handshakes", well short of "the tools work". The five tools judged most likely to behave differently in practice than on paper have all since been confirmed hands-on: `computer` (including its elevated-window/UIPI caveat), `speak`/`listen` (real TTS played out loud, real mic capture correctly transcribed, both end-to-end), and `document_convert`/`interactive_run` (a real markdown file converted to a real PDF via pandoc+soffice, and a real interactive prompt answered end-to-end via a spawned PowerShell script). Still worth your own sanity-check in your specific environment, but no longer purely CI-green-and-hope.
...
```powershell
python mcp_client.py            # add --help for the inspector's other modes
```
...
Per-tool dependencies are imported **lazily, inside the tool that needs them** — this does not mean skippable at install time: `requirements.txt` installs every one of them unconditionally. The lazy import just means that if a package were ever missing anyway (e.g. a stale venv), only that one tool breaks — it still gets declared to Claude and returns an install hint if used, instead of crashing the whole client at startup. To drop a tool entirely, remove its module from `MODULES` in `core/local_tools.py`.
...
- **`[embeddings]` in config.toml** — settings for the `text_embeddings` tool
  (`core/text_embeddings.py`): `url` (required — the tool errors by name until this is
  set), `model`, `request_format` (`"openai"` default or `"simple"`), `api_key_env`
  (same `token_env`-style indirection as the MCP bearer tokens above — never the token
  itself), and `timeout`. Entirely commented out by default, and read fresh from disk on
  every call rather than cached at import, unlike `[mcp].servers` and `[claude].model`
  which `main.py` reads once at startup.
- **`[vision]` in config.toml** — settings for the `vision_query` tool
  (`core/vision.py`): `url` (required — the tool errors by name until this is set),
  `model`, `max_tokens` (default 4000 — tuned for a reasoning-capable vision model that
  can burn a low budget entirely on invisible `reasoning_content` before writing its real
  answer), `timeout` (default 180), and `api_key_env` (same indirection as
  `[embeddings].api_key_env` above). Also entirely commented out by default and read
  fresh from disk on every call. **No automatic fallback to Claude's own vision lives in
  this tool** — if the server is unset or unreachable, it returns a `local_unavailable`
  status and stops; using Claude's own vision on the same image after that is a separate,
...
- **`main.py`** — entrypoint (in the repo root). Reads the API key, builds a `Claude` service, connects every enabled `[mcp].servers` entry over Streamable HTTP via `build_client()` / `_connect_mcp_servers()` (which runs `_expand_paths()` on each entry first, so `%USERNAME%`/`~` in paths resolve) — a server that fails is reported and skipped rather than aborting startup — plus any stdio servers passed as argv, wires everything into a `Chat`, and runs the `CliApp` loop. An argv-passed stdio server is spawned with **`sys.executable`, never a bare `"python"`**: on Windows PATH usually resolves that either to the App Execution Alias that opens the Microsoft Store (a stub that exits without running anything) or to a system interpreter that is not this venv. **No event-loop policy is set** — the upstream `WindowsProactorEventLoopPolicy` call was already a no-op, since Proactor has been the Windows default since 3.8, and `set_event_loop_policy` is deprecated as of 3.14 and slated for removal in 3.16. Proactor is what is wanted (Selector has no subprocess support, which the stdio MCP transport needs) and `asyncio.run` already provides it. Three things are registered on the `AsyncExitStack`, in an order that matters: `process_reaper`'s check is pushed FIRST specifically so it runs LAST (the stack unwinds LIFO) — after every MCP client's own `cleanup()` and `local_tools.shutdown()` have already had their chance, since it's a safety net for what THEY might have missed, not a replacement for either.

- **`core/process_reaper.py`** — `reap_orphans()`: the last-line safety net `main.py` runs on exit, independent of what any tool's own cleanup claims to have handled. Windows has neither `/proc` (the Linux original's own mechanism) nor a `ps` binary (the macOS port's), so discovery goes through WMI instead — `Get-CimInstance -ClassName Win32_Process | Select-Object ProcessId, ParentProcessId, Name`, shelled out to whichever of `pwsh`/legacy `powershell.exe` `core/powershell.py`'s own `_powershell_executable()` would find (duplicated here, not imported — this fork's established convention). `ParentProcessId` was chosen over the newer cross-platform `System.Diagnostics.Process.Parent` property specifically because the latter was tried first and found unreliable: confirmed live, on a real PowerShell 7 process, that bulk `Get-Process` leaves `.Parent` null for nearly every entry (including the calling process's own), while `Get-CimInstance`'s `ParentProcessId` is the plain, long-established field every "map a Windows process tree" script already uses. **Genuinely unverified on non-Windows dev hardware, same honesty standard as `powershell_session.py`'s own pywinpty/ConPTY transport**: the real WMI query has never run for real here (`Get-CimInstance` simply errors "term not recognized" outside Windows) — what HAS been verified, via `test_process_reaper.py`'s `raw_json` injection hook, is everything downstream of that call: parsing a `Win32_Process`-shaped payload (including `ConvertTo-Json`'s own bare-object-not-array quirk for a single result), the self-referential-subprocess-pid filter (the invoking PowerShell process briefly appears as its own child at the exact moment it captures the snapshot — same artifact the macOS `ps` port hit live), children-map construction, recursive descendant collection, and — exercised against REAL spawned process trees on the dev sandbox — that killing every discovered pid via `os.kill(pid, signal.SIGTERM)` (which CPython documents as calling Win32 `TerminateProcess` unconditionally for any non-`CTRL_*` signal — itself also unverified on real Windows) actually leaves the tree genuinely empty afterward, not just self-reported as killed. No zombie-reaping step, unlike the Linux/macOS originals — deliberately: Windows has no equivalent to a POSIX zombie for a process this one didn't itself spawn via `subprocess.Popen`, and `os.waitpid()` isn't even exposed by CPython's `os` module on this platform.
...
- **`core/powershell_session.py`** — `powershell_session`: persistent `pwsh` (`cd`/`$env:`/variables/functions/modules survive across calls), this fork's counterpart to Linux/Router's `core/bash_session.py` and Mac's `core/zsh_session.py`, built on **pywinpty/ConPTY** (the same library `interactive_run` uses) rather than `pexpect` (which cannot spawn on Windows at all) — reuses `core/processes.py`'s own `_Reader`-polling-a-queue technique, duplicated rather than imported (this fork's stand-alone-module convention). Not a port: PSReadLine (PowerShell's line editor) differs enough from bash's readline or zsh's ZLE to need its own design. **PSReadLine fires an ANSI cursor-position query (`\x1b[6n`) at startup and periodically after, and stalls without a reply** — the read loop (`_pump_until`) answers every occurrence with a synthetic `\x1b[1;1R`, continuously, for the life of the session (no zsh-`unsetopt zle`-style one-liner exists to suppress the query instead). **A bare `\n` sent to an interactive PSReadLine session is silently dropped**, not inserted and not treated as Enter — only `\r` submits a line, which rules out sending a multi-line construct as one payload the way `bash_session` does. The fix for that also answers the scoping question: each command is staged into a temp `.ps1` file (ordinary file, real newlines, no pty quirks) with a reset/exit-code trailer appended to the same file, and exactly one line is ever typed live: `. '<path>'` — dot-sourcing runs in the *caller's* own scope (a variable, `Set-Location`, and a function defined inside all persist afterward), and since the trailer is the file's last lines it always runs before the host calls `prompt()` again (PowerShell's analog of `PROMPT_COMMAND`/`precmd`, invoked before every top-level read even after a script redefines it). Exit-code capture needs its own formula: `$LASTEXITCODE` is native-command-only and **sticky** (a later successful cmdlet does not reset it), so the trailer only consults it when `$?` itself says the last thing failed (`$rc = if (-not $?) { if ($LASTEXITCODE) { $LASTEXITCODE } else { 1 } } else { 0 }`). **Timeout recovery does not attempt Ctrl-C at all, unlike bash_session/zsh_session** — `\x03` does not propagate into anything executing inside the dot-sourced staging path (a `Start-Sleep`/busy-loop under it just runs to completion regardless; a bare, top-level typed command still interrupts normally, but that's not the path any real command here takes). So `_handle_timeout()` skips straight to force-kill + full respawn on every timeout; `state_reset: true` unconditionally, honestly, since there is no "plain recovery keeps state" case this transport can support. **Two things remain genuinely unverified, not just unexplored**: whether the Ctrl-C non-propagation is specific to the pexpect/PowerShell-Core transport used to validate this design or also holds on real ConPTY (whose Ctrl+C delivery — `GenerateConsoleCtrlEvent` — is architecturally different from Unix's tty-byte-to-SIGINT), and the `pywinpty`/ConPTY transport itself — `pywinpty` is a Windows-only wheel and cannot be exercised on Linux at all, so whether real ConPTY already answers the DSR query before this module's own loop ever sees it needs a real Windows run to settle. See the module's own docstring for the underlying design in full.
...
- **`core/processes.py`** — `interactive_run`: **pywinpty/ConPTY** on a real pseudo-console for commands that prompt (passwords, `[y/N]`, ssh host keys, winget and other installers, REPLs), which the `powershell` tool cannot answer and simply hangs on. One tool taking a scripted list of expect/send steps rather than a stateful spawn/send/expect trio. pywinpty's `read()` is blocking-only with no timeout, so a reader thread feeds a queue that this module polls against a per-step deadline. Four things worth knowing, all found by reading pywinpty's source rather than by testing:
  ...
  - **A step's answer can come from three places**: `send` (literal, model writes it directly), `send_env` (an environment variable NAME only), or `send_secret` (a `gopass` entry NAME only) — the latter two exist for the same reason as the Linux/Mac originals: a literal `send` for a real secret round-trips the value through Anthropic's API twice (told to the model, then written into its own tool call) before `secret: true` ever gets a chance to redact anything, which only ever covered what came back afterward. `send_secret` uses **`gopass`**, not the original `pass` — real `pass` is a bash script with no native Windows build at all, `gopass` is a from-scratch Go reimplementation explicitly marketed as *"a drop-in replacement for pass"*, ships a genuine native Windows binary (`winget install gopass.gopass`, alongside `winget install GnuPG.Gpg4win`, its own documented Windows setup), and mirrors the exact subcommands needed (`show <name>`, `ls --flat`) closely enough that `_resolve_reply`/`_select_entry_prompt` need no Windows-specific branch beyond the binary name. Windows Credential Manager (`cmdkey`/DPAPI) was evaluated and rejected for the same reason the Mac fork rejected Keychain and the Linux original rejected `secret-tool`: a second, platform-specific credential backend when this project's own convention (`config.toml`'s `token_env`/`api_key_env` indirection) already commits to `pass`-compatible tooling everywhere. `_select_entry_prompt()` (the `"?"`-sentinel / first-reference-refusal prompt, same `_confirmed_secret_entries`-gated mechanism as the other two forks) uses `gopass ls --flat` rather than walking a store directory by hand the way the Linux/Mac originals do for `pass` — `gopass` has no single guaranteed store location the way `pass` guarantees `~/.password-store`, and `--flat` already prints one full entry path per line with no tree-drawing to work around. `resolved_replies` are resolved for every step up front, before `PtyProcess.spawn` is ever called, exactly like the Linux/Mac up-front-resolution shape — and `secrets_to_scrub` (the module's own existing live incoming-buffer scrub, see the point above) is seeded from ALL of them at that same point, before spawn, rather than appended step-by-step as each is sent: a value is scrubbed from incoming buffer content from the very first chunk read onward, not merely after the step that actually sends it. `test_processes.py` covers all of this (missing var, missing/unconfirmed/absent `gopass` entry, the confirmation gate, timeout, the redact-even-when-echoed-later regression) by faking BOTH `gopass` (a tiny shell script on `PATH`, same technique as the Linux/Mac suites) AND `winpty` itself (a `pexpect`-backed `/bin/bash` stand-in installed into `sys.modules['winpty']`, since real ConPTY genuinely cannot run off a real Windows box) — meaning it exercises the real, unmodified `_run()` for real, but does not and cannot verify anything ConPTY/cmd.exe-specific; that half needs a real Windows run, same limitation the module's own docstring and this fork's CI already accept elsewhere.
...
- **`core/text_embeddings.py`** — `text_embeddings`: vector embeddings from whatever
  HTTP server the `[embeddings]` block in `config.toml` names, since Anthropic has no
  first-party embeddings endpoint of its own (the documented path is Voyage AI, a
  separate paid API this deliberately avoids requiring). Does nothing until `url` is
  set — declared to Claude either way, same pattern as `sql_query` when `duckdb` is
  missing, and returns a named error rather than crashing. `request_format` picks
  between an OpenAI-compatible POST body (`{"input": [...], "model": ...}`, the default,
  matching most self-hosted servers — llama.cpp's server, Ollama, vLLM, LM Studio,
  text-embeddings-inference — and Voyage AI's own paid endpoint unmodified) and a
...
- **`core/vision.py`** — `vision_query`: asks a question about an image via whatever
  OpenAI-compatible vision-capable chat server the `[vision]` block in `config.toml`
  names, instead of spending an Anthropic API vision call. The `image` field accepts a
  local file path, an `http(s)://` URL, or an already-encoded `data:` URI — auto-detected
  by prefix, no separate fields needed; a local path is read, base64-encoded, and wrapped
  as a `data:` URI before the request goes out. `max_tokens` defaults to 4000 rather than
  a smaller number because reasoning-capable vision models (this was tuned against
  GLM-4.6V-Flash) can spend the entire token budget on invisible `reasoning_content` and
  return an empty or mid-sentence-truncated visible answer with no error, only
  `finish_reason: "length"` in the raw response to explain why. **Deliberately has no
  fallback path to Claude's own vision inside the tool itself** — on failure (server
  unset, unreachable, timeout) it returns a plain `{"status": "local_unavailable", ...}`
  with the original `image`/`prompt` echoed back, and stops; using Claude's own vision on
...
- **`core/speak.py`** — `speak`: text-to-speech through a local Piper voice model,
  ...
  name substring (`python -m sounddevice` to list devices); not required, since Windows
...
- **`core/listen.py`** — `listen`: records from the microphone and transcribes it
  ...
  in-process, with `soundfile` writing the temp WAV. `device` **does not need to be set** —
...
- **`mcp_server.py`** — the opposite direction: serves this agent to an MCP client (Claude Code, or a ResearchMesh-Router worker entry) as two tools, so the app is a server and a client at once. **`delegate(task, session, thinking)`** is the whole agent in one call. Four things decide its shape. **One tool, not 24** — `memory_20250818` and `computer_20251124` are learned schemas, and `computer` needs the `computer-use-2025-11-24` header on the request that *declares* it; that header belongs to this app's own API call, so re-exporting those tools over MCP would strip the trained schema. Wrapping `Chat.run()` keeps them intact and keeps `SYSTEM_PROMPT` in force, which is why the shell/editor overlap with the caller's own tools is deliberate rather than redundant, and why the ~30-50 tool ceiling doesn't apply. **It `chdir`s to the repo root**, because a client spawns it with the client's project as cwd and `CLAUDE_MEMORY_DIR` defaults to a *relative* `memories`. **Calls are serialised behind an `asyncio.Lock`** — one mouse, one browser page, one kernel. One `Chat` is kept per `session` id, so the caller can follow up on a previous delegation; a new id starts clean. The second tool, **`model(action, arg)`**, is a direct list/swap of which Claude model THIS worker uses for `delegate` — no agent turn spent, no Anthropic API call made, just a mutation of the same module-level `_claude.model` every `delegate` session already reads off of. It mirrors this worker's own `/model`/`/model swap` command exactly (same `load_claude_models`/`resolve_model_swap` calls from `core/claude.py`), just reachable over MCP instead of typed at this machine's own prompt — this is what lets a caller like ResearchMesh-Router swap a connected worker's model remotely, not just its own.
