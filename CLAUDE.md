...
**This is ResearchMesh-Windows, a fork of [ResearchMesh](https://github.com/nodormu/ResearchMesh) taken at commit `9e6959b`, whose goal is a Windows port. The port has not started.** No code in this repo differs from upstream yet, so every statement below still describes the Linux original and remains accurate about *what the code currently does* — it is simply not yet accurate about the target platform. The known Linux-bound surface is `core/processes.py` (POSIX-only `pexpect`), `core/computer.py` (X11/Wayland gating), `core/kernel.py` (ZeroMQ `ipc://`, which Windows lacks), `core/claude_learned_schemas.py` (the `bash` tool), `core/documents.py` (the `soffice` path), and the `apt`-based setup instructions throughout this file and `README.md`. `.github/workflows/ci.yml` still runs on `ubuntu-latest`. Don't "fix" a Linux assumption in passing — the port is a deliberate piece of work, not a cleanup.
