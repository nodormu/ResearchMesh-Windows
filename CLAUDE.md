...
- **`core/powershell_session.py`** — `powershell_session`: persistent `pwsh` (`cd`/`$env:`/variables/functions/modules survive across calls), this fork's counterpart to Linux/Router's `core/bash_session.py` and Mac's `core/zsh_session.py`, built on **pywinpty/ConPTY** (the same library `interactive_run` uses) rather than `pexpect` (which cannot spawn on Windows at all) — reuses `core/processes.py`'s own `_Reader`-polling-a-queue technique, duplicated rather than imported (this fork's stand-alone-module convention). Not a port: PSReadLine (PowerShell's line editor) behaves nothing like bash's readline or zsh's ZLE, and three things had to be discovered live against a real `pwsh` process (validated on Linux, since PowerShell 7 is the same interpreter cross-platform — see the module's own docstring for the full research trail) before this could be written at all. **(1)** PSReadLine fires an ANSI cursor-position query (`\x1b[6n`) at startup and periodically after, and will not proceed — corrupting every later command into garbled ParserErrors — unless something answers it; unlike zsh's one-line `unsetopt zle` fix, there is no way to make it stop asking, so the read loop (`_pump_until`) answers every occurrence with a synthetic `\x1b[1;1R`, continuously, for the life of the session. **(2)** a bare `\n` typed into an interactive PSReadLine session is silently DROPPED (not inserted, not treated as Enter), which rules out sending a multi-line construct as one payload the way `bash_session` does. **(3)** the fix for that is also what answers the scoping question: every command is staged into a temp `.ps1` file (own real newlines, ordinary script parsing, no pty quirks apply) with a reset/exit-code trailer appended to the same file, and exactly one line is ever typed live: `. '<path>'` — dot-sourcing runs in the *caller's* scope (confirmed live: a variable, a `Set-Location`, and a function defined inside all persisted), and because the trailer is the last lines of that same file it always runs before the host can call `prompt()` again (confirmed to be the exact analog of `PROMPT_COMMAND`/`precmd`, including the same "still fires after a script redefines it" worst case). Exit-code capture is its own formula, not a direct port: `$LASTEXITCODE` is native-command-only and **sticky** (a later successful cmdlet does not reset it), so the trailer only consults it when `$?` itself says the last thing failed (`$rc = if (-not $?) { if ($LASTEXITCODE) { $LASTEXITCODE } else { 1 } } else { 0 }`). **Timeout recovery does NOT attempt Ctrl-C at all, unlike bash_session/zsh_session** — confirmed live, decisively, that `\x03` does not propagate as a stop request into anything executing inside the dot-sourced staging path above: sent against a running `Start-Sleep -Seconds 90` inside a dot-sourced script with a 100s response window, the session answered at t=90.2s, the full natural sleep duration — Ctrl-C had zero effect. A tight busy-loop showed the identical non-response, ruling out "just one cmdlet's polling behavior." (A bare, top-level TYPED command still interrupts via Ctrl-C fast and cleanly, ~1s — that's a separate, still-true finding, just not the path any real command here takes.) So `_handle_timeout()` skips straight to force-kill + full respawn on every timeout, no health-check probe, no `os.tcgetpgrp`-style "confirmed vs. looks recovered" distinction to even attempt — `state_reset: true` unconditionally, honestly, since there is no "plain recovery keeps state" case this transport can support. **Unverified whether the Ctrl-C non-propagation is a PowerShell-Core-on-Linux artifact of this research transport or also true on real Windows/ConPTY** — Windows' actual Ctrl+C delivery (`GenerateConsoleCtrlEvent`) is a real console event, architecturally different from Unix's tty-byte-to-SIGINT translation this finding was based on. **Genuinely unverified either way**: the actual `pywinpty`/ConPTY transport in production — `pywinpty` is a Windows-only wheel and cannot be exercised on Linux at all, so real Windows Console behavior (does ConPTY already answer the DSR query itself, before this module's own loop ever sees it? does Ctrl-C behave differently there?) needs a real Windows run to settle. In the `shutdown()` tuple.
